...
void main()
{
	...
	s1.o = view * (vec3(-0.75f, 0.5f, 3.0f) - position);
	s1.color = vec3(0.5f, 0.5f, 1.0f);
	s1.r = 0.5f;
	s1.type = 1;

	s2.o = view * (vec3(0.75f, 0.5f, 3.0f) - position);
	s2.color = vec3(1.0f, 0.5f, 0.5f);
	s2.r = 0.5f;
	s2.type = 1;

	p.i = view * (vec3(0.0f, 0.0f, 0.0f) - position);
	p.j = view * (vec3(1.0f, 0.0f, 0.0f) - position);
	p.k = view * (vec3(0.0f, 0.0f, 1.0f) - position);
	p.color = vec3(0.5f, 0.5f, 0.5f);
	p.type = 2;
	...
	for (int t = 0; t < ref_count; t++)
	{
		...
		vec3 point_p = planeIntersect(p, ro, rd);

		sky = true;
		light_source = false;

		if (point_s1 != vec3(0.0f))
		{
			point = point_s1;
		...
		if (point_p != vec3(0.0f) && (length(point_p - ro) < length(point - ro) || sky))
		{
			point = point_p;
			n = planeNorm(p);
			color = p.color;
